...
public class StorageService {
    ...
    public String uploadImage(MultipartFile file, String authorization, Integer forehead, Integer leftCheek, Integer rightCheek, Integer chin) throws IOException {
        Optional<Member> memberInformation = memberRepository.findByEmail(jwtTokenToPayload(authorization));

        if (memberInformation.isPresent()) {
            Member member = memberInformation.get();

            // FacePictureService를 이용하여 FacePicture 저장
            FacePicture savedFacePicture = facePictureService.saveFacePicture(
                    member,
                    forehead,
                    leftCheek,
                    rightCheek,
            ...
            // 저장된 FacePicture를 이용하여 ImageData 저장
            ImageData imageData = storageRepository.save(
                    ImageData.builder()
                            .name(file.getOriginalFilename())
                            .type(file.getContentType())
                            .imageData(ImageUtils.compressImage(file.getBytes()))
                            .facePicture(savedFacePicture)  // 이미지 데이터와 FacePicture 매핑
                            .build());
    ...
    public byte[] downloadImage(String fileName) {
        ...
        return ImageUtils.decompressImage(imageData.getImage_data());
    }
